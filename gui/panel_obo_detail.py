...
from application.define import EnumOBOType,EnumOBODataType
from application.class_observable import Observable


class OBODetailPanel(wx.Panel):
    def __init__(self, parent):
        wx.Panel.__init__(self, parent, wx.ID_ANY)
        self.mainSizer = wx.BoxSizer(wx.VERTICAL)
        self._formSizer = wx.GridBagSizer(5, 5)
        self._detailTitle = wx.StaticText(self, wx.ID_ANY, 'ObservableObjectDetail')
        self._detailTitle.SetFont(wx.Font(10, wx.FONTFAMILY_SCRIPT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD))
        self._dataLvLabel = wx.StaticText(self, wx.ID_ANY, 'ObservableObjectData:')
        ...
        _col_type_render = dv.DataViewChoiceRenderer(EnumOBODataType.ALL.value)
    ...
    def on_cm_add(self, evt):
        self.dvlc.AppendItem(('NewData', EnumOBODataType.STRING.value, ''))

    def on_cm_del(self, evt):
        _row = self.dvlc.GetSelectedRow()
        self.dvlc.DeleteItem(_row)

    def set_data(self, data: Observable):
    ...
    def get_data(self):
        ...
        for i in range(self.dvlc.GetItemCount()):
            _obo_data.append({'name': self.dvlc.GetTextValue(i, 0),
